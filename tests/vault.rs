...
use minizeke::{
    ...
    pool::deploy_pool,
    test_utils::{
        get_client, get_funded_user, get_vault, register_user_on_vault, send_note_to_network,
    },
...
async fn test_fund_redeem() -> Result<()> {
    ...
    let pool = deploy_pool(&mut client, vault_id).await?;
    let pool_id = pool.id();
    ...
    .await?;
    let user_balance_before_fund = client
    ...
    send_note_to_network(&mut client, &fund_note, user_id).await?;
